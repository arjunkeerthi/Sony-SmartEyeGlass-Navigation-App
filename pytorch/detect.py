...
def limit_remote_addr():
    ip = ['129.59.122.14', '10.66.78.206', '127.0.0.1', '129.59.105.162']
...
def upload():
    if request.method == 'POST':
        fh = open("img/"+request.values.get('name'), "wb")
        fh.write(base64.b64decode(request.values.get(('image'))))
        fh.close()
        result = process(request.values.get('name'))
        os.remove(fh.name)
        print(result)
        return result
        """req_data = request.get_json()
        fh = open("img/"+req_data['name'], "wb")
        fh.write(base64.b64decode(req_data['image']))
        fh.close()
        result = process(req_data['name'])
        #os.remove(fh.name)
        print(result)
        return result
"""
...
def process(image):
    #Detection phase
    try:
        imlist = [osp.join(osp.dirname(__file__), "img\\"+image)]
        #print(imlist)
        #imlist = [osp.join(osp.realpath('.'), images, img) for img in os.listdir(images) if
    # os.path.splitext(img)[1] == '.png' or os.path.splitext(img)[1] =='.jpeg' or os.path.splitext(img)[1] =='.jpg']
    ...
    for im_num, image in enumerate(imlist[i*batch_size: min((i +  1)*batch_size, len(imlist))]):
        ...
        print(str(end-start) + " time taken to process image")
    ...
    #for x in output:
     #   result+=("{0}".format(classes[int(x[-1])]))
